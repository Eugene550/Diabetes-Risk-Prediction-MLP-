...
# ============================================================
print("\n[SAVING RESULTS] Writing metrics to model_results.txt")
print("-" * 40)

with open('model_results.txt', 'w', encoding='utf-8') as f:
    f.write("=" * 60 + "\n")
    f.write("  DIABETES RISK PREDICTION — MODEL RESULTS\n")
    f.write("  BAXI 3133 | ANN (MLP) with Keras/TensorFlow\n")
    f.write("=" * 60 + "\n\n")

    # Dataset info
    f.write("DATASET (after cleaning)\n")
    f.write("-" * 40 + "\n")
    f.write(f"Total records : {df.shape[0]:,}\n")
    f.write(f"Features used : {X.shape[1]}\n")
    f.write(f"Train / Val / Test : {X_train.shape[0]} / {X_val.shape[0]} / {X_test.shape[0]}\n\n")

    # Cross-validation results
    f.write("STRATIFIED 5-FOLD CROSS VALIDATION\n")
    f.write("-" * 40 + "\n")
    for i, (a, u) in enumerate(zip(cv_acc, cv_auc), 1):
        f.write(f"  Fold {i}: Accuracy = {a:.4f} | ROC-AUC = {u:.4f}\n")
    f.write(f"  Mean Accuracy : {np.mean(cv_acc):.4f} (+/- {np.std(cv_acc):.4f})\n")
    f.write(f"  Mean ROC-AUC  : {np.mean(cv_auc):.4f} (+/- {np.std(cv_auc):.4f})\n\n")
    ...
    # Confusion matrix
    cm_save = confusion_matrix(y_test, y_test_pred)
    f.write("CONFUSION MATRIX (Test Set)\n")
    f.write("-" * 40 + "\n")
    f.write(f"  True Negative : {cm_save[0,0]}\n")
    f.write(f"  False Positive: {cm_save[0,1]}\n")
    f.write(f"  False Negative: {cm_save[1,0]}\n")
    f.write(f"  True Positive : {cm_save[1,1]}\n\n")
